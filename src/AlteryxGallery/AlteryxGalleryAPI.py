...
if __name__==__main__:
    ...
    class GalleryAdmin(Gallery):
        def __init__(self, api_location: str, admin_api_key: str, admin_api_secret: str):
            api_location_base = api_location.rsplit("/", 1)
            self.api_location = api_location_base[0]+ "/admin/" + api_location_base[1] + "/"
            self.admin_api_key = admin_api_key
            self.admin_api_secret = admin_api_secret
        ...
        def get_workflows_migratable(self):
            """
            Generate a list of workflows that have been set as migratable
            :return: Returns a list of workflows
            """
            method = 'GET'
            url = self.api_location + '/' + app_id + '/package/'
            params = self.build_oauth_params()
