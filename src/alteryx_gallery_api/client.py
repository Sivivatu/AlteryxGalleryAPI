...
class AlteryxClient:
    ...
    def __init__(
        ...
        Args:
            base_url (str): The base URL of the Alteryx Server instance 
                            (e.g., 'http://localhost/webapi/' or 'https://your.alteryxserver.com/webapi/').
                            It should typically end with '/webapi/'.
            api_key (str): The API Access Key obtained from the user profile.
            api_secret (str): The API Access Secret obtained from the user profile.
        ...
        try:
            ...
            if not all([result.scheme, result.netloc, result.path]):
                raise ValueError("Invalid base_url: Must include scheme, domain, and path (e.g., https://your-gallery-url/webapi/).")
        except Exception as e:
            raise ValueError(f"Invalid base_url: {e}")

        self.base_webapi_url = base_url
        ...
        try:
            ...
            if e.status_code == 404: # Handle 404 from PUT specifically
                 raise WorkflowNotFoundError(
                    workflow_id,
                    message=f"Failed to update: Workflow ID '{workflow_id}' not found."
                ) from e
    ...
    def delete_workflow(self, workflow_id: str) -> None:
        ...
        try:
            ...
            if e.status_code == 404:
                raise WorkflowNotFoundError(
                    workflow_id,
                    message=f"Failed to delete: Workflow ID '{workflow_id}' not found."
                ) from e
    ...
    def queue_job(
        ...
        try:
            ...
            if e.status_code == 404:
                raise WorkflowNotFoundError(
                    workflow_id,
                    message=f"Failed to queue job: Workflow ID '{workflow_id}' not found."
                ) from e
        ...
        try:
            ...
            return response
             ...
             if e.status_code == 404:
                 ...
                 raise JobExecutionError(
                     f"Job ID '{job_id}' or Output ID '{output_id}' not found.",
                     job_id=job_id,
                     status_code=404
                 ) from e
